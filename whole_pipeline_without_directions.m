    ...
    if d(i).isdir
        ...
        for e = 1:length(FullEEG.event)
            ...
            if countblocks == 2 || countblocks == 3 || countblocks == 4
                if contains (FullEEG.event(e).type,'11')
                    FullEEG.event(e).type = '10 ';
                end
            else
                if contains (FullEEG.event(e).type,'11') || ...
                   contains (FullEEG.event(e).type,'10')
        ...
        END = 0.5;
        %data epoching
        data_pro{i} = pop_epoch(FullEEG, {'10  '}, [BEGIN,END]);
        data_anti{i} = pop_epoch(FullEEG, {'12  '} , [BEGIN,END]);
  
        data_pro{i} = pop_rmbase(data_pro{i},1000*[BEGIN,0]);
        data_anti{i} = pop_rmbase(data_anti{i},1000*[BEGIN,0]);
        
         %I'm interested only in saccades with latencies [100;500]ms ale nie
        %dziala
        data_pro{i} = FilterByEventLatency(data_pro{i}, 'L_saccade_10', 100, 500);
        data_anti{i} = FilterByEventLatency(data_anti{i}, 'L_saccade_12', 100, 500);
...
for i=1:length(data_pro)%dlaczego, przeciez one moga miec rozne dlugosci...
    ...
    ITEMS = {
        {data_pro{i}, 'L_saccade_10', 'PRO ', [-500,500]}
        {data_anti{i}, 'L_saccade_12', 'ANTI ', [-500,500]}
    };
    I_DATA = 1;
    I_TRIGGER = 2;
    I_TITLE = 3;
...
for i=1:length(data_pro_yng)
    [data_pro_yng{i}, pi] =ClipToBounds(data_pro_yng{i}, -500, 500);
    [data_anti_yng{i}, ai] = ClipToBounds(data_anti_yng{i}, -500, 500);
    
    [data_pro_mpe_yng{i}, pi_cell] = clip_tb_cell(data_pro_mpe_yng{i}, -500, 500);
    [data_anti_mpe_yng{i}, ai_cell] = clip_tb_cell(data_anti_mpe_yng{i}, -500, 500);
    
end

for i=1:length(data_pro_old)
    [data_pro_old{i}, pi] =ClipToBounds(data_pro_old{i}, -500, 500);
    [data_anti_old{i}, ai] = ClipToBounds(data_anti_old{i}, -500, 500);
    
    [data_pro_mpe_old{i}, pi_cell] =clip_tb_cell(data_pro_mpe_old{i}, -500, 500);
    [data_anti_mpe_old{i}, ai_cell] = clip_tb_cell(data_anti_mpe_old{i}, -500, 500);
...
ITEMS = {
    {data_pro_yng, 'L_saccade_10', 'PRO  YNG'}
    {data_anti_yng, 'L_saccade_12', 'ANTI  YNG'}
    
	{data_pro_old, 'L_saccade_10', 'PRO OLD'}
	{data_anti_old, 'L_saccade_12', 'ANTI OLD'}
